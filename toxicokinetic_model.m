...
clear all;
close all;
Q = 10000; % number of iterations for Monte Carlo analysis
...
% Statistics: geomean and 2.5% lower bound and 97.5% upper bound
Mean_blood = (mean(Blood'))';
Upperbound_blood = (prctile(Blood',97.5))';
Lowerbound_blood = (prctile(Blood',2.5))';
...
% Plot for Plasma time concentration profile
figure(1);
semilogy(t,Mean_blood,'--r','LineWidth',2);
xlabel('Time(day)');
ylabel('Concentration(ng/g)');
title('Plasma time concentration profile');
hold on
semilogy(t,Upperbound_blood,'--g','LineWidth',2);
hold on
semilogy(t,Lowerbound_blood,'--b','LineWidth',2);
hold on